...
class ExtraPropertyFix:
    ...
    def applyFix(self, property_list, index):
        ...
        if 'value_alias' in self.alias:
            ...
            property_list[index]['value'] = self.alias['value_alias'].get(
                value, value)

        if 'copy_nearest' in self.alias:
            matching_property = self.searchPropertyByMatch(
                property_list, index)
...
def ApplyCharacterFixes(m):
    ...
    propertyFixList = [ExtraPropertyFix(
        alias) for alias in characterFixes[character_name]['extraproperty']]
...
def applyGlobalRequirementAliases(requirement_list):
    requirementAliasList = [GlobalRequirementFix(
        key) for key in globalRequirementsReplace]
